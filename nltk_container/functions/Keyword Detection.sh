...
# Send a test request to the keyword extraction endpoint and save the response
response=$(curl --fail -s -X POST -H "Content-Type: application/json" -d "{\"text\": \"$text\"}" http://localhost:$port/keywords)
...
if echo "$response" | grep -q "error"; then
    echo "Error occurred during keyword extraction:"
    echo "$response"
    docker logs $container_id >> "$BASE_DIR/outputs/logs.txt"
    exit 1
fi

# Save the keyword extraction results to a file
echo "$response" > "$BASE_DIR/outputs/keywords_results.txt"
...
cat "$BASE_DIR/outputs/keywords_results.txt"
