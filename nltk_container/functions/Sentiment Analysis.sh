...
# Send a test request to the sentiment analysis endpoint and save the response
response=$(curl --fail -s -X POST -H "Content-Type: application/json" -d "{\"text\": \"$text\"}" http://localhost:$port/sentiment)
...
if echo "$response" | grep -q "error"; then
    echo "Error occurred during sentiment analysis:"
    echo "$response"
    docker logs $container_id >> "$BASE_DIR/outputs/logs.txt"
    exit 1
fi
...
echo "$response" > "$BASE_DIR/outputs/sentiment_results.txt"
...
cat "$BASE_DIR/outputs/sentiment_results.txt"
