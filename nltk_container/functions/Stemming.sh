...
# Send a test request to the stemming endpoint and save the response
response=$(curl --fail -s -X POST -H "Content-Type: application/json" -d "{\"text\": \"$text\"}" http://localhost:$port/stem)
...
if echo "$response" | grep -q "error"; then
    echo "Error occurred during stemming:"
    echo "$response"
    docker logs $container_id >> "$BASE_DIR/outputs/logs.txt"
    exit 1
fi

# Save the stemming results to a file
echo "$response" > "$BASE_DIR/outputs/stem_results.txt"

# Append container logs to logs.txt without writing to stdout
#docker logs $container_id >> "$BASE_DIR/outputs/logs.txt" 2>&1

echo "Stemming completed successfully."

# Display the stemming results
echo "Stemming results:"
cat "$BASE_DIR/outputs/stem_results.txt"
