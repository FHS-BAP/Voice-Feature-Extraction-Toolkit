...
response=$(curl --fail -s -X POST -H "Content-Type: application/json" -d "{\"text\": \"$text\"}" http://localhost:5000/keywords)
...
echo "$response" > "$BASE_DIR/results/keywords_results.txt"
