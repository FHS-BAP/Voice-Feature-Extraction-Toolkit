...
response=$(curl --fail -s -X POST -H "Content-Type: application/json" -d "{\"word\": \"$word\"}" http://localhost:5000/antonyms)
...
echo "$response" > "$BASE_DIR/results/antonyms_results.txt"
