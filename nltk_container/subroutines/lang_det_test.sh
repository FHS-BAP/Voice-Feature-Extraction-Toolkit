...
response=$(curl --fail -s -X POST -H "Content-Type: application/json" -d "{\"text\": \"$text\"}" http://localhost:5000/detect_language)
...
echo "$response" > "$BASE_DIR/results/language_results.txt"
