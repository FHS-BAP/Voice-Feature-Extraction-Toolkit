...
response=$(curl --fail -s -X POST -H "Content-Type: application/json" -d "{\"text\": \"$text\"}" http://localhost:5000/word_freq)
...
echo "$response" > "$BASE_DIR/results/word_freq_results.txt"
