...
response=$(curl --fail -s -X POST -H "Content-Type: application/json" -d "{\"text\": \"$text\"}" http://localhost:5000/remove_stopwords)
...
echo "$response" > "$BASE_DIR/results/remove_stopwords_results.txt"
