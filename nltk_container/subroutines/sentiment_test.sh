...
response=$(curl --fail -s -X POST -H "Content-Type: application/json" -d "{\"text\": \"$text\"}" http://localhost:5000/sentiment)
...
echo "$response" > "$BASE_DIR/results/sentiment_results.txt"
