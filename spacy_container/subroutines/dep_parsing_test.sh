...
response=$(curl --fail -s -X POST -H "Content-Type: application/json" -d "{\"text\": \"$text\"}" http://localhost:5000/dep_parse)
...
echo "$response" > "$BASE_DIR/results/dep_parsing_results.txt"
