...
response=$(curl --fail -s -X POST -H "Content-Type: application/json" -d "{\"text\": \"$text\"}" http://localhost:5000/tokenize)
...
echo "$response" > "$BASE_DIR/results/tokenization_results.txt"
