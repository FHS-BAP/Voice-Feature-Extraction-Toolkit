...
response=$(curl --fail -s -X POST -H "Content-Type: application/json" -d "{\"text\": \"$text\"}" http://localhost:$port/tokenize)
...
if echo "$response" | grep -q "error"; then
    echo "Error occurred during tokenization:"
    echo "$response"
    docker logs $container_id >> "$BASE_DIR/outputs/logs.txt"
    exit 1
fi

# Save the tokenization results to a file
echo "$response" > "$BASE_DIR/outputs/tokenization_results.txt"
...
cat "$BASE_DIR/outputs/tokenization_results.txt"
