...
# Send a test request to the SBD endpoint and save the response
response=$(curl --fail -s -X POST -H "Content-Type: application/json" -d "{\"text\": \"$text\"}" http://localhost:$port/sbd)
...
if echo "$response" | grep -q "error"; then
  echo "Error occurred during sentence boundary detection:"
  echo "$response"
  docker logs $container_id >> "$BASE_DIR/outputs/logs.txt"
  exit 1
fi

# Save the SBD results to a file
echo "$response" > "$BASE_DIR/outputs/sbd_results.txt"

# Append container logs to logs.txt without writing to stdout
#docker logs $container_id >> "$BASE_DIR/outputs/logs.txt" 2>&1

echo "Sentence boundary detection completed successfully."

# Display the SBD results
echo "Sentence boundary detection results:"
cat "$BASE_DIR/outputs/sbd_results.txt"
