...
# Send a test request to the NER endpoint and save the response
response=$(curl --fail -s -X POST -H "Content-Type: application/json" -d "{\"text\": \"$text\"}" http://localhost:$port/ner)
...
if echo "$response" | grep -q "error"; then
  echo "Error occurred during named entity recognition:"
  echo "$response"
  docker logs $container_id >> "$BASE_DIR/outputs/logs.txt"
  exit 1
fi

# Save the NER results to a file
echo "$response" > "$BASE_DIR/outputs/ner_results.txt"

# Append container logs to logs.txt without writing to stdout
#docker logs $container_id >> "$BASE_DIR/outputs/logs.txt" 2>&1

echo "Named Entity Recognition completed successfully."

# Display the NER results
echo "Named Entity Recognition results:"
cat "$BASE_DIR/outputs/ner_results.txt"
