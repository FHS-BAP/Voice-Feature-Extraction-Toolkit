...
# Send a test request to the stopword removal endpoint and save the response
response=$(curl --fail -s -X POST -H "Content-Type: application/json" -d "{\"text\": \"$text\"}" http://localhost:$port/remove_stopwords)
...
if echo "$response" | grep -q "error"; then
    echo "Error occurred during stopword removal:"
    echo "$response"
    docker logs $container_id >> "$BASE_DIR/outputs/logs.txt"
    exit 1
fi

# Save the stopword removal results to a file
echo "$response" > "$BASE_DIR/outputs/remove_stopwords_results.txt"

# Append container logs to logs.txt without writing to stdout
#docker logs $container_id >> "$BASE_DIR/outputs/logs.txt" 2>&1

echo "Stopword removal completed successfully."

# Display the stopword removal results
echo "Stopword removal results:"
cat "$BASE_DIR/outputs/remove_stopwords_results.txt"
