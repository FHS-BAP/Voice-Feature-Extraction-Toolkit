...
# Send a test request to the word frequency distribution endpoint and save the response
response=$(curl --fail -s -X POST -H "Content-Type: application/json" -d "{\"text\": \"$text\"}" http://localhost:$port/word_freq)
...
if echo "$response" | grep -q "error"; then
    echo "Error occurred during word frequency distribution:"
    echo "$response"
    docker logs $container_id >> "$BASE_DIR/outputs/logs.txt"
    exit 1
fi

# Save the word frequency distribution results to a file
echo "$response" > "$BASE_DIR/outputs/word_freq_results.txt"

# Append container logs to logs.txt without writing to stdout
#docker logs $container_id >> "$BASE_DIR/outputs/logs.txt" 2>&1

echo "Word frequency distribution completed successfully."

# Display the word frequency distribution results
echo "Word frequency distribution results:"
cat "$BASE_DIR/outputs/word_freq_results.txt"
